...
function DNSNode(host, _settings) {
  ...
  const _debugHeartbeat = (0, _debug3.default)('dnsmq-messagebus:dnsnode:heartbeat');
  ...
  let electionTimeout = settings.electionTimeout,
  ...
  let _masterBroker = (0, _MasterMessagesBroker2.default)({ id: _id });
  let _nodesUpdater = (0, _ExternalNodesUpdater2.default)(settings);
  ...
  _electionCoordinator.on('newMaster', newMaster => {
    ...
    if (!_connected || newMaster.endpoints.sub !== _connectedMaster.endpoints.sub || newMaster.endpoints.pub !== _connectedMaster.endpoints.pub) {
  ...
  let _connectToMaster = () => {
    ...
    debug('Connecting to new master: ', (0, _utils.nodeIdToName)(_connectedMaster.id));
    ...
    _newIntSub.on('message', function (channelBuffer) {
      ...
      if (channel === 'heartbeats') {
        ...
        if (_connectedMaster.name === master.name) {
          _nodesUpdater.publish.apply(_nodesUpdater, [channelBuffer].concat(argsBuffers));
        }
        return;
      }
